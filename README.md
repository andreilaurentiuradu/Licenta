...
├── scripts/
│   ├── seed.py                     # Demo accounts + ~6 months of mock data, then triggers FL per club (run.sh seed)
│   ├── demo_risk.py                # FL demo: target a player's risk to low/medium/high or reset (run.sh risk)
│   ├── realistic_dataset.py        # Turn the synthetic CSV into a realistic (imbalanced + noisy) one
│   └── requirements.txt
├── graphics/                       # Thesis presentation assets
│   ├── prezentare.html             # 14-slide deck (open in browser / print to PDF)
│   ├── LawrAnalyzer.pptx           # editable PowerPoint (generated by build_pptx.py)
│   ├── build_pptx.py               # builds LawrAnalyzer.pptx (python-pptx)
│   ├── grafice_prezentare.py       # generates the 3 chart PNGs
│   └── grafic_1/2/3_*.png          # survey · accuracy vs baseline · FL convergence
├── notebooks/
...
> After a fresh `start` (empty volumes — new DB + Keycloak), run `./run.sh seed` once to populate accounts + data. The DB services use SQLAlchemy `pool_pre_ping`, so the first request after a Postgres restart no longer fails with a stale-connection error.
